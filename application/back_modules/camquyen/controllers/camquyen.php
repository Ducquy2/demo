...
class camquyen extends MX_Controller
{
function __construct() {
    parent::__construct();
    //Khai bao model hien tai
    $this->load->model('model_camquyen',"Mdl_camquyen",TRUE);
...
public function index()
	{
	        //Goi ham trong model thong qua bien khai bao tren
                $data['detail']=  $this->Mdl_camquyen->get('1');
                            
                //Cho du lieu : teen model va ten view vao mang $data
                $data['module']="camquyen";
...
public function addedit($id = false){
    ...
    if(intval($id)){  // Update
        ...
        $getpic = $this->Mdl_camquyen->get_where($id);
...
//                $data_post1 = $this->Mdl_camquyen->do_upload(); 
//                $data_post['hinh']=$data_post1['hinh'];
//                $data_post['path']=$data_post1['path'];
//                
//                $this->Mdl_camquyen->_update($id, $data_post);
//            }
//            // lấy nội dung theo $id
//             $data['detail'] = $this->Mdl_camquyen->get_where($id);
...
//                    $this->Mdl_camquyen->_insert($data_post);
//                     
//                 }
        }
       ...
       //Kiem tra neu bam nut submit thi chuyen huong ve index.php
        /* @var $_POST type */
        if ($_POST) {
            redirect(base_url() . "admin/camquyen");
        }
    }
