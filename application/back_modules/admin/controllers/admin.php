...
function __construct() {
    ...
    $this->load->model('model_admin',"Mdl_admin",TRUE);
...
public function index($id = false)
	{
          // Phan trang       
                 ...
                 $config['total_rows']=  $this->Mdl_admin->count_all();
                 $config['per_page']=10;
                 $config['num_links']=5; 
              ...
              // End Phan trang
                ...
                //$data['detail']=  $this->Mdl_admin->get('1');
                 //echo $this->uri->segment(2); exit;    
                $data['detail']= $this->Mdl_admin->get_with_limit($config['per_page'],$id,1);
...
public function addedit($id = false){
        ...
        $this->load->model('model','Mdl_admin');
        $data_post = $this->input->post();
        
      
        if(intval($id)){  // Update
           if($data_post){    // Click Submit
               $this->Mdl_admin->_update($id, $data_post);
            }
            // lấy nội dung theo $id
             $data['detail'] = $this->Mdl_admin->get_where($id);
        }else{
            // Insert
             if($data_post){    // Click Submit
                     ...
                     $this->Mdl_admin->_insert($data_post);
...
public function delete($id) {
       //Xoa hinhf trong thu muc
        $getpic = $this->Mdl_admin->get_where($id);
...
public function search()
{
    ...
    $data['detail']= $this->Mdl_admin->get_where_custom_like('ten',$data_post['search']);
