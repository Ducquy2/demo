...
class khachhang extends MX_Controller
{
function __construct() {
    parent::__construct();
    //Khai bao model hien tai
    $this->load->model('model_khachhang',"Mdl_khachhang",TRUE);
...
public function index($id = false)
	{
          // Phan trang       
                 ...
                 $config=array();
                 $config['base_url']=  base_url().'admin/khachhang/index';
                 $config['total_rows']=  $this->Mdl_khachhang->count_all();
                 $config['per_page']=10;
                 $config['num_links']=5; 
              ...
              // End Phan trang
                ...
                //$data['detail']=  $this->Mdl_khachhang->get('1');
                 //echo $this->uri->segment(2); exit;    
                $data['detail']= $this->Mdl_khachhang->get_with_limit($config['per_page'],$id,1);
                 
                //Cho du lieu : teen model va ten view vao mang $data
                $data['module']="khachhang";
 ...
 public function addedit($id = false){
        ...
        $this->load->model('model','Mdl_khachhang');
        $data_post = $this->input->post();
        
      
        if(intval($id)){  // Update
           if($data_post){    // Click Submit
               $this->Mdl_khachhang->_update($id, $data_post);
            }
            // lấy nội dung theo $id
             $data['detail'] = $this->Mdl_khachhang->get_where($id);
        }else{
            // Insert
             if($data_post){    // Click Submit
       ...
       if($_POST)
            redirect(base_url()."admin/khachhang");
    }
public function delete($id) {
       //Xoa hinhf trong thu muc
        $getpic = $this->Mdl_khachhang->get_where($id);
...
public function search()
{
    ...
    $data['detail']= $this->Mdl_khachhang->get_where_custom_like('ten',$data_post['search']);
...
public function changestatus($id=false, $trangthai)
{
    ...
    $this->Mdl_khachhang->_update($id,$data);     
