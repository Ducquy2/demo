...
class hoadon extends MX_Controller
{
function __construct() {
    parent::__construct();
    //Khai bao model hien tai
    $this->load->model('model_hoadon',"Mdl_hoadon",TRUE);
...
public function index()
	{
	        //Goi ham trong model thong qua bien khai bao tren
                $data['detail']=  $this->Mdl_hoadon->get('1');
...
public function addedit($id = false){
    ...
    if(intval($id)){  // Update
        ...
        $getpic = $this->Mdl_hoadon->get_where($id);
...
//                $data_post1 = $this->Mdl_hoadon->do_upload(); 
//                $data_post['hinh']=$data_post1['hinh'];
//                $data_post['path']=$data_post1['path'];
//                
//                $this->Mdl_hoadon->_update($id, $data_post);
//            }
//            // lấy nội dung theo $id
//             $data['detail'] = $this->Mdl_hoadon->get_where($id);
...
//                    $this->Mdl_hoadon->_insert($data_post);
//                     
//                 }
        }
...
public function delete($id) {
       //Xoa hinhf trong thu muc
        ...
        $this->Mdl_hoadon->_delete($id);
        redirect(base_url()."admin/hoadon");
}
public function changestatus($id=false, $trangthaihd)
{
    $data[trangthaihd]= $trangthaihd;
    $this->Mdl_hoadon->_update($id,$data);     
